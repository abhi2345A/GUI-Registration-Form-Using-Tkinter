...
class Registration_Form:
    ...
    def insert(self):
            ...
            self.sheet.cell(row=current_row + 1, column=1).value = self.params[0]
            self.sheet.cell(row=current_row + 1, column=2).value = self.params[1]
            self.sheet.cell(row=current_row + 1, column=3).value = self.params[2]
            self.sheet.cell(row=current_row + 1, column=4).value = self.params[3]+self.params[4]
            self.sheet.cell(row=current_row + 1, column=5).value = self.params[5]
            self.sheet.cell(row=current_row + 1, column=6).value = self.params[8]
            self.sheet.cell(row=current_row + 1, column=7).value = self.params[7]
    ...
    def mainget(self):
        ...
        mail=  self.entry_3.get()
        code=self.ck.get()
        YeaR=self.c2.get()
        MontH=self.c3.get()
        DatE=self.c4.get()
        mob= self.entry_4b.get()
        GendR=self.var.get()
        Pass =self.entry_7.get()
        Conf_Pass= self.entry_8.get()
        ...
        self.params = [fname,lname,mail,code,mob,GendR,Pass, Conf_Pass,date_string,YeaR,MontH,DatE]
    ...
    def layout(self):
        ...
        background_label = Label(image=self.pic)
        background_label.pack()
        background_label.place(x=0, y=0, relwidth=1, relheight=1)
        background_label.image = self.pic
        ...
        self.entry_1= Entry(canvas,bg="white",bd=4)
        self.entry_1.place(x=180,y=150)
        label2=Label(canvas,text="Last Name",font=("Times",14),fg="black",bg="snow3",)
        label2.place(x=350,y=150)
        self.entry_2= Entry(canvas,bg="white",bd=4)
        self.entry_2.place(x=470,y=150)
        label3=Label(canvas,text="Email ",font=("Times",14),fg="black",bg="snow3",)
        label3.place(x=60,y=220)
        self.entry_3= Entry(canvas,bg="white",width=50,bd=4)
        self.entry_3.place(x=180,y=220)
        ...
        droplist=OptionMenu(canvas,self.ck,*list1)
        self.ck.set('+91')
        droplist.place(x=180,y=290)
        self.entry_4b= Entry(canvas,bg="white",width=39,bd=4)
        self.entry_4b.place(x=260,y=290)
        label_5 = Label(canvas, text="Gender",font=("Times", 14),fg="black",bg="snow3")
        label_5.place(x=60,y=360)

        Radiobutton(canvas, text="Male",padx = 10, variable=self.var, value=1,bg="snow3").place(x=180,y=360)
        Radiobutton(canvas, text="Female",padx = 20, variable=self.var, value=2,bg="snow3").place(x=250,y=360)
        ...
        droplist2=OptionMenu(canvas,self.c2,*list2)
        self.c2.set("Year")
        ...
        droplist3=OptionMenu(canvas,self.c3,*list3)
        self.c3.set("Month")
        ...
        droplist4=OptionMenu(canvas,self.c4,*list4)
        self.c4.set("Date")
        droplist4.place(x=200,y=430)

        label7=Label(canvas,text="Password",font=("Times",14),fg="black",bg="snow3",)
        label7.place(x=60,y=500)
        self.entry_7= Entry(canvas,bg="white",width=50,show="*",bd=4)
        self.entry_7.place(x=180,y=500)
        ...
        self.entry_8= Entry(canvas,bg="white",width=50,show="*",bd=4)
        self.entry_8.place(x=180,y=570)
...
def main():
    ...
    try:
        ...
        sheet = wb.active
        obj=Registration_Form(root,sheet)
        obj.layout()
        root.mainloop()
        obj.excel()
        obj.insert()
